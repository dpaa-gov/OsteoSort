pm.input <- function(bone = NULL, ref = NULL, sort = NULL, measurements = NULL, threshold = 1) {
    ...
    measurements <- measurements[cb[seq_along(measurements)]]
    ...
    if (nrow(refleft) == 0 || nrow(refright) == 0) {
        return(NULL)
    }
    ...
    if (nrow(sortleft_t) == 0 || nrow(sortright_t) == 0) {
        return(NULL)
    }
    ...
    for (i in seq_len(nrow(sortleft_t))) {
    ...
    for (i in seq_len(nrow(sortright_t))) {
    ...
    if (nrow(sortleft) == 0 || nrow(sortright) == 0) {
        return(NULL)
    }
